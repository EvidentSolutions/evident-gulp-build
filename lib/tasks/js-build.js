...
exports.registerTasks = function(gulp) {
    ...
    gulp.task('js:build:libs', ['init'], function () {
        return compileLibs(settings.debug);
    });

    gulp.task('js:build:app', ['init', 'angular-templates:build'], function () {
        return compileAppJs('app.js', false, false, settings.debug);
    });

    gulp.task('js:build:bundles', ['js:build:libs', 'js:build:app']);

    gulp.task('js:build:bundle', ['init', 'angular-templates:build'], function () {
        return compileAppJs('bundle.js', false, true, settings.debug);
    });

    gulp.task('js:build', ['js:build:bundles']);

    gulp.task('js:watch', ['js:build:libs'], function () {
